...
library("LambertW")    # For transforming the phenotypes with Gaussianize function
...
if(names(phenotypes)[1] != "id"){print("ERROR: id column header has to be \"id\" and it is not!!!")}
...
## TRANSFORMATION!!! (VERY IMPORTANT STEP!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
...
#    pheno[,2] <- NULL
  ...
  manhattan(gwasResults, cex=1.6, cex.axis=1.6, cex.lab=1.8, col=man.pal, main=trait,
  ...
  par(mar = c(5, 5.2, 4, 2) + 0.1) # Add space for the y axes lable
  qq(gwasResults$P, cex = 1.2, cex.axis = 1.5, cex.lab=1.6, col="darkblue")
  dev.off()
}
