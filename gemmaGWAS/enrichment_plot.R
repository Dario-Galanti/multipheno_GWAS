...
ggplot(enrichment, aes(x=X.logP)) +
  ...
  theme(
    ...
    axis.title.y = element_text(color = "dodgerblue4", hjust = 0.5, size=24),
    axis.text.y = element_text(color="dodgerblue4", size=21),
    axis.title.y.right = element_text(color = "firebrick", hjust = 0.5, vjust = 1.5, size=24),
    axis.text.y.right = element_text(color="firebrick", size=21)
  )
dev.off()
