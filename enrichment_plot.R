...
enrichment$FDR <- ifelse(is.na(enrichment$FDR), 1, enrichment$FDR) # Set FDR to 1 if enrichment = 0

## 3a)  PRINT ENRICHMENT AND FDR PLOT
max_enrich <- ceiling(max(enrichment$Enrichment, 10))
...
ggplot(enrichment, aes(x=X.logP)) +
  ...
  scale_y_continuous(
    name = "Enrichment", limits = c(0,max_enrich),
    sec.axis = sec_axis( trans=~./max_enrich, name="FDR")
  ) +
  #theme_minimal() +
  theme_light() +
  theme(
    axis.title.x = element_text(hjust = 0.5, size=23),
    axis.text.x = element_text(size=20),
    axis.title.y = element_text(color = "dodgerblue4", hjust = 0.5, size=23),
    axis.text.y = element_text(color="dodgerblue4", size=20),
    axis.title.y.right = element_text(color = "firebrick", hjust = 0.5, vjust = 1.5, size=23),
    axis.text.y.right = element_text(color="firebrick", size=20)
  )
dev.off()
