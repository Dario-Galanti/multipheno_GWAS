...
###         4) Individual zygosity report based on varinats used for IBS matrix
...
## Substitute "_" with "-" in sample names (or plink crashes) and add variant name (3rd column of vcf)
zcat $fin | awk 'OFS="\t"{if(!/\#/){$3=$1":"$2;print} else {gsub("_","-");print}}' > ${wDir}/temp_ready.vcf
...
### 5) Summary
echo "Report from vcfGWAformat_IBSmx.sh, input file was:" $fin > $filter_report
