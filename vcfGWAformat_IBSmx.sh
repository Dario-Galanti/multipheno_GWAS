...
###         6) Individual zygosity report based on varinats used for IBS matrix
...
## 3)           Scaffold_n --> n+7 (eg. Scaffold_1 --> 8) in order not to re-use Chr numbers
...
#$Rscript --vanilla ../ibs_plot.R $pruned_base.mibs $pruned_base.mibs.id

### 2) Strict MAF filtering (0.04), format for GWAS
...
### 5) Summary
echo ""
echo "Report from vcfGWAformat_IBSmx.sh, this script starts from already imputed 10NA 1MAF filtered variants"
echo $(wc -l ${fin_base}.map | cut -d" " -f1) "variants after 0.1NA, biallelic, 1MAF filtering"
echo $(wc -l ${ibs_base}.map | cut -d" " -f1) "variants after 0.1NA, biallelic, 1MAF filtering and pruning (--indep-pairwise 50 5" $pruneLD_IBS"). These were used for IBS matrix"
echo $(wc -l ${gwa_base}.map | cut -d" " -f1) "variants after 0.1NA, biallelic," $MAF"MAF filtering. These variants are ready for GWAS"
echo $(wc -l ${ind_tests}.prune.in | cut -d" " -f1) "variants after 0.1NA, biallelic," $MAF"MAF filter and pruning (--indep-pairwise 100 5 0.3). This is the number of independent tests in GWAS"


### 6) OPTIONAL: Count homozygous, hetero and... sites (using pruned SNPs used for the IBS matrix)
$plink --file $ibs_base --allow-extra-chr --recode A --out $ibs_base
awk 'OFS="\t"{if(NR==1){print "sample","homo_ref","het","homo_alt"}
 else{href=0;het=0;halt=0;for(i=7;i<=NF;i++){if($i==0){href++}else if($i==1){het++}else{halt++}};print $1,href,het,halt}}' $ibs_base > zigosity_report.txt
